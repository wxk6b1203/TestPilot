...
> ⚠️ **本项目处于活跃开发与测试阶段，接口与行为可能随时调整，可能存在 bug。**
> 欢迎试用与反馈问题，生产使用请自行评估并锁定版本。
...
![Copilot 对话页](./assets/screenshot_3.png)

![Copilot 生成低代码用例（HITL 审批）](./assets/screenshot_4.png)

### 接口管理

- HTTP 接口：方法 / URI / params / headers / cookies / body / TLS / 重定向 / JSONC / 二进制引用；
  单接口调试工作区（即时请求与响应面板）。
- 设计/调试分离：参数类型、必填、默认值、说明等预设，请求/响应体 JSON Schema；
  调试可按设计一键回填，并勾选是否发送 params/headers/cookies。
- gRPC 接口：proto 文件资产 + server reflection 动态调用，无需预编译桩。
- 目录树、项目 / 环境 / 变量 / 证书管理；变量支持 `{{expr}}` 模板与敏感标记。
  数据模型（「结构」）定义可复用的 JSON Schema。
...
- **回复语言**跟随界面语言（每次请求经 `X-TP-Lang` 头下发）；
  `TP_COPILOT_DEFAULT_LANGUAGE` 配置默认语言（`zh`）。
...
- Web 控制台支持**中英文**：通过右上角（登录页也在右上角）的地球图标切换，
  选择持久化在 `localStorage`（`tp_lang`）。
- 后端（scheduler / worker / copilot 工具面）返回**统一英文**消息并带稳定错误码
  （见 `docs/error-codes.md`）；所有面向展示的本地化都在前端完成。
  `GET /api/v1/meta/locale` 可查询后端消息语言（`en-US`）。
