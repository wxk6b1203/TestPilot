...
func seed(d *gorm.DB) error {
	...
	if cnt == 0 {
		...
		if adminPw == "" {
			...
			// 也会静默落到这里，不能只依赖运维看文档。
			fmt.Fprintln(os.Stderr, "[WARN] TP_ADMIN_PASSWORD not set; admin account created with default password admin123 - set a strong password immediately in production")
		}
