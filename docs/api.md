...
- `POST /cases/{id}/run`（member）：单用例直接运行 → `{run_id}`；body 可选 `{"env_id": n}`，缺省取项目第一个环境。
...
| GET/POST | /api-tokens | admin | CI/CLI 机器凭证；POST 返回一次性 `token`（tp_ 前缀），库中仅存哈希 |
| DELETE | /api-tokens/{id} | admin | 删除后使用中的 token 立即失效 |
